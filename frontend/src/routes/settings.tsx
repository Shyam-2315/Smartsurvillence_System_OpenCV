...
function SettingsPage() {
  ...
  const { data: rule } = useQuery({ queryKey: ["rule-loitering"], queryFn: fetchLoiterRule, refetchInterval: 10000 });
        ...
        <div className="glass space-y-4 rounded-2xl p-5">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between rounded-lg border border-border/60 bg-background/30 p-3">
              <span className="text-sm text-muted-foreground">Alert when a tracked person stays too long</span>
              <Switch
                checked={enabled}
                onCheckedChange={(v) => {
                  setDirty(true);
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Zones are normalized (0..1) polygons. Default: Full Frame.</p>
          </div>
          <Button
