  ...
  return (
    <div className="glass flex h-full flex-col rounded-2xl">
      ...
      <div className="flex-1 space-y-2 overflow-y-auto p-3">
        {isLoading && Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-16 w-full rounded-xl" />)}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={cn("rounded-md border px-1.5 py-0.5 text-[10px] uppercase tracking-widest", severityStyles[a.severity])}>
                      {a.severity}
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {a.camera ?? "Camera"} • {formatDistanceToNow(new Date(a.timestamp), { addSuffix: true })}
                  </p>
                </div>
              </motion.div>
            );
