...
export function ImagesGallery() {
  const { data, isLoading } = useQuery({ queryKey: ["images"], queryFn: fetchImages, refetchInterval: 10000 });
  ...
  return (
    <div className="glass rounded-2xl p-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {isLoading && Array.from({ length: 10 }).map((_, i) => <Skeleton key={i} className="aspect-square rounded-xl" />)}
        {(data ?? []).map((img, i) => (
          <motion.button
            key={img.filename}
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-background/0 to-transparent opacity-0 transition-opacity group-hover:opacity-100">
              <div className="absolute inset-x-0 bottom-0 flex items-center justify-between p-2">
                <span className="text-[10px] text-muted-foreground">{format(new Date(img.timestamp), "HH:mm")}</span>
        ...
        <DialogContent className="max-w-4xl border-primary/30 bg-background/95 p-0">
          {active && (
            <div className="relative">
              <img src={imageUrl(active.filename)} alt={active.filename} className="max-h-[80vh] w-full rounded-md object-contain" />
              <div className="flex items-center justify-between border-t border-border/60 p-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{format(new Date(active.timestamp), "PPpp")}</p>
                </div>
